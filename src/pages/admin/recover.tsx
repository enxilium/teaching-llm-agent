...
export default function RecoverPage() {
    ...
    const [recoverResults, setRecoverResults] = useState<Array<{success: boolean; error?: string; key?: string}>>([]);
    ...
    const recoverBackup = async (key: string, data: unknown) => {
        try {
            ...
            const experimentData = dataObj.data || dataObj.flowData || data;

            // Save to Firebase with extra recovery metadata
            const recoveryData = {
                ...(experimentData as object),
                _recovery: {
            ...
            const result = await saveDataToFirebase(recoveryData as unknown as import('@/utils/types').ExperimentData);

            // Track results
            setRecoverResults((prev) => [
                ...prev,
                { key, success: true, result },
            ]);
    ...
    const recoverAllBackups = async () => {
        ...
        setMessage(
            `Recovery complete. Successful: ${results.successful}, Failed: ${results.failed}`
        );
    };
    ...
    }, [authenticated, scanLocalStorage]);
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            ...
            {message && (
                <p className="mb-4 p-2 bg-gray-100 rounded">{message}</p>
            )}
            ...
            <div>
                <h2 className="text-xl font-semibold mb-2">
                    Available Backups ({Object.keys(backups).length})
                </h2>
