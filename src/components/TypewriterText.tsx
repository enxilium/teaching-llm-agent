...
export default function TypewriterText({
    ...
    const createNaturalChunks = useCallback((text: string): string[] => {
    ...
    const startAnimation = useCallback(() => {
        ...
        const animate = (time: number) => {
            ...
            const interval =
                baseInterval *
                randomFactor *
                (currentChunk.length > 15 ? 1.2 : 0.8);
            ...
            if (elapsed > interval) {
                ...
                if (chunkIndexRef.current < chunksRef.current.length) {
                    setDisplayText(
                        (prev) =>
                            prev + chunksRef.current[chunkIndexRef.current]
                    );
        ...
        animationRef.current = requestAnimationFrame(animate);
    }, [isComplete, onCharacterTyped, onComplete, speed]);
    ...
    }, [text, skip, createNaturalChunks, startAnimation]);
