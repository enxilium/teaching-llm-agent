...
export default function SinglePage() {
    ...
    // Timer for work phase (before answer submission)
    useEffect(() => {
    ...
    // Timer for chat phase (after answer submission)
    useEffect(() => {
        if (!hasSubmittedAnswer || roundEndedRef.current) return;
    ...
    return (
        <div className="h-screen bg-gradient-to-b from-[#2D0278] to-[#0A001D] p-4 flex flex-row overflow-hidden fixed inset-0">
            <div className={`${hasSubmittedAnswer ? "w-1/2 pr-2" : "w-full max-w-4xl mx-auto"} flex flex-col h-full overflow-hidden`}>
                ...
                {/* Answer Input Section - only show before submission */}
                {!hasSubmittedAnswer && (
            ...
            {/* Chat Panel - only show after submission */}
            {hasSubmittedAnswer && (
                <SingleScenarioChat
                    agents={agents}
